...
public class FenJeu extends JFrame {
	...
	public JPanel SkillP(){
	/*	JPanel panel=new JPanel();
		JPanel panEast=new JPanel(new BorderLayout());
		JPanel panCenter=new JPanel(new BorderLayout());
		JPanel panCenterMid=new JPanel();
		panCenter.setBorder(BorderFactory.createTitledBorder("Skills"));
		JPanel panEastTop=new JPanel();
		JPanel panEastBot=new JPanel();
		panEastTop.setPreferredSize(new Dimension(400,200));
		JButton back=new JButton("Back");
		back.setPreferredSize(new Dimension(200,100));
		panEastTop.add(this.Cons);
		panEastBot.add(back);
		panEast.add(panEastTop,BorderLayout.NORTH);
		panEast.add(panEastBot,BorderLayout.SOUTH);
		for(int i=0; i<this.character.getSkills().size();i++){
			panCenterMid.add(new JButton(this.character.getSkills().get(i).getName()));
		}
		panCenter.add(panCenterMid,BorderLayout.CENTER);
		panel.add(panCenter,BorderLayout.CENTER);
		panel.add(panEast,BorderLayout.EAST);*/
		PanelImage pan= new PanelImage("FondStart.jpg");
		pan.setLayout(new BorderLayout());
		JPanel panLeft=new JPanel();
		JPanel panRight=new JPanel(new BorderLayout());
		...
		back.addActionListener(list);
		back.setPreferredSize(new Dimension(200,100));
		panel2.add(back,BorderLayout.CENTER);
		panRight.add(panel1, BorderLayout.CENTER);
		panRight.add(panel2,BorderLayout.SOUTH);
		panLeft.setOpaque(false);
		panel2.setOpaque(false);
	...
	public JPanel FightP(){
		...
		p1.setText("HP = "+fight.getHP1()+"/"+fight.getC1().getVIT()+"\n"+fight.getC1());
		p2.setText("HP = "+fight.getHP2()+"/"+fight.getC2().getVIT()+"\n"+fight.getC2());
	...
	public class FightListener implements ActionListener{
		public void actionPerformed(ActionEvent event){
			...
			if(s.equals("Attack with Weapon"))
				fight.setAction1(fight.AttackWeapon(fight.getC1()));
			else if(s.equals("Attack with Shield"))
				fight.setAction1(fight.AttackShield(fight.getC1()));
			else if(s.equals("Defend with Weapon"))
				fight.setAction1(fight.DefendWeapon(fight.getC1()));
			else if(s.equals("Defend with Shield"))
				fight.setAction1(fight.DefendShield(fight.getC1()));
			else if(s.equals("Use Potion"))
				fight.setAction1(fight.UsePot(fight.getC1()));
			
			else if(s.equals("Escape")){
				fight.setCombatFini(true);
				fight.setSurrender(FenJeu.this.character);
			}
			
			for(int i=0; i< character.getSkills().size();i++){
				if(s.equals(character.getSkills().get(i).getName())){
					fight.setAction1(fight.UseSkill(fight.getC1(), i));
					FenJeu.this.setContentPane(FenJeu.this.FightP());
				}
			}
			if(s.equals("Use Skill"))
				FenJeu.this.setContentPane(FenJeu.this.UseSkillP());
				...
				System.out.println(fight.getC1().getShi().getEFF());
			...
			else if(ite==0 && fight.getCombatFini()==false){
	...
	public class SkillListener implements ActionListener{
		public void actionPerformed(ActionEvent event){
			...
			for(int i=0; i< character.getSkills().size();i++){
				if(s.equals(character.getSkills().get(i).getName())){
					Cons.setText(character.getSkills().get(i).toString());
				}
			}
			if(s.equals("Back")){
				if(character==null)
	...
	public class ButtonListener implements ActionListener{
		public void actionPerformed(ActionEvent event){
			...
			if(s.equals("Show Skills"))
				FenJeu.this.setContentPane(FenJeu.this.SkillP());
			if(s.equals("Upgrade")){
				try{
			...
			if(s.equals("Fight")){
				Character Ene=null;
				fight=new Fight(FenJeu.this.character,Character.charger("Sauvegarde4",Ene));
				FenJeu.this.setContentPane(FenJeu.this.FightP());
				ite=fight.setFight();
				p1.setText("HP = "+fight.getHP1()+"/"+fight.getC1().getVIT()+"\n"+fight.getC1());
				p2.setText("HP = "+fight.getHP2()+"/"+fight.getC2().getVIT()+"\n"+fight.getC2());
